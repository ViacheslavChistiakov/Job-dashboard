<template>
  <div v-if="isOpen" class="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
    <div
      class="dark:bg-gray-800 w-full max-w-lg max-h-[90vh] overflow-y-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden bg-white rounded-2xl shadow-2xl p-6"
    >
    <div class="dark:bg-gray-800 w-full flex flex-col sticky top-0 z-10 bg-white/100">
      <div class="w-full  flex flex-row justify-between items-center">
        <h2 class="text-xl font-bold">
          {{ job?.title }}
        </h2>
        <button class="mt-6" @click="emit('close')">❌</button>
      </div>
